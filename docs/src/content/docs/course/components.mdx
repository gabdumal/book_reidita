...
Uma característica chave de jogos _Idle Clicker_ é um botão em que o jogador pode clicar para ganhar pontos.
No nosso jogo, teremos mais de um botão, cada um para ganhar um tipo de recurso diferente.

### Grupo de botões

Podemos imaginar que esses botões serão agrupados de alguma forma.
...
1. Dentro da pasta `src/components`, crie uma pasta `Resources`.
2. Dentro dela, crie um arquivo `Resources.tsx`.
...
4. Crie um arquivo `Resources.module.css` para estilizar o componente.
...
6. Importe o componente `Resources` no arquivo `App.tsx`, e o adicione logo abaixo do componente `Header`.
...
Depois de criar o elemento que agrupa os botões, podemos de fato criar os recursos do jogo.
Para isso, criaremos outro componente, chamado `Resource`.

1. Dentro da pasta `src/components`, crie uma pasta `Resource`.
2. Dentro dela, crie um arquivo `Resource.tsx` e o preencha com o seguinte código:
...
3. Crie um arquivo `Resource.module.css` para estilizar o componente e adicione a ele as seguintes regras de estilo:
...
4. Abra o arquivo `Resources.tsx` e remova o texto "Recursos".
5. Crie um elemento `<div>` com a classe `styles.group` e adicione o componente `Resource` dentro dele.
...
O resultado final será um botão com o emoji de uma árvore em que o jogador pode clicar.

<Figure
  caption={`Componentes "Resources" e "Resource.tsx" estilizados.`}
  images={[
    {
      src: resources,
      alt: `Página do projeto, mostrando um botão com o ícone de uma árvore e o texto "Madeira" acima. Abaixo, um número zero indica a quantidade de madeira.`,
    },
  ]}
/>
