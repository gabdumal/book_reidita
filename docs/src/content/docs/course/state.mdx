---
title: Estado
author: Gabriel Malosto
---

import { Aside } from "@astrojs/starlight/components";
import Figure from "/src/components/Figure.astro";

import actions from "/src/assets/content/docs/course/state/actions.png";
import trades from "/src/assets/content/docs/course/state/trades.png";
import newResources from "/src/assets/content/docs/course/state/new_resources.png";
import newActions from "/src/assets/content/docs/course/state/new_actions.png";

Agora que temos os botões, precisamos implementar suas ações.
A lógica de coletar madeira é simples: ao clicar no botão, a quantidade de madeira deve aumentar.
Concluímos que deve haver uma variável para armazenar a quantidade de madeira.

A implementação trivial seria criar uma variável global para armazenar a quantidade de madeira.
No entanto, isso não é uma boa prática, pois variáveis globais podem causar problemas de manutenção e escalabilidade.

Além disso, podemos melhorar essa ideia ao não somente armazenar a quantidade, mas também representar cada recurso como um objeto.

## Tipos

Quando trabalhamos com objetos no TypeScript, é uma boa prática antes definir uma interface que represente a forma do objeto.
Uma vez que esses tipos serão usados por todo o projeto, é melhor defini-los em um arquivo separado.

### Recurso

1. Crie um arquivo chamado `types.ts` na pasta `src`.
2. Adicione as seguintes interfaces ao arquivo:

```tsx
/* types.ts */

export interface Resource {
  name: string;
  icon: string;
  amount: number;
}

export interface Resources {
  wood: Resource;
  coin: Resource;
}
```

3. Edite o componente `ResourceDisplay` para que ele receba um objeto do tipo `Resource`. Lembre-se de importar a interface `Resource` no início do arquivo.

```tsx
/* ResourceDisplay.tsx */

interface ResourceDisplayProps {
  resource: Resource;
}
```

4. Atualize os props recebidos pela função que define o componente. Também atualize o conteúdo dos elementos `<h1>` e `<span>` para exibir o nome, o ícone e a quantidade do recurso.

```tsx
/* ResourceDisplay.tsx */

export default function ResourceDisplay({
  resource,
  amount,
}: ResourceDisplayProps) {
  return (
    <div className={styles.box}>
      <h1 className={styles.name}>{resource.name}</h1>
      <span className={styles.icon}>{resource.icon}</span>
      <p className={styles.amount}>{resource.amount}</p>
    </div>
  );
}
```

5. Dentro da **função** que define o componente `ResourcesContainer`, crie um objeto do tipo `Resource` para representar a madeira e outro para representar as moedas.

```tsx
/* ResourcesContainer.tsx */

export default function ResourcesContainer() {
  const wood: Resource = {
    name: "Madeira",
    icon: "🌳",
    amount: 0,
  };

  const coin: Resource = {
    name: "Moeda",
    icon: "🪙",
    amount: 0,
  };

  // ...
}
```

6. Logo abaixo, crie um objeto do tipo `Resources` que inclua a madeira e as moedas.

```tsx
/* ResourcesContainer.tsx */

const resources: Resources = {
  wood,
  coin,
};
```

7. Atualize as chamadas do componente `ResourceDisplay` para que elas passem o recurso.

```tsx
/* ResourcesContainer.tsx */

return (
  <section className={styles.container}>
    <div className={styles.group}>
      <ResourceDisplay resource={resources.wood} />
      <ResourceDisplay resource={resources.coin} />
    </div>
  </section>
);
```

A página deve continuar exibindo os recursos da mesma forma que antes.
Apenas mudamos a forma como os recursos são representados no código.

### Ação

Vamos atualizar a representação das ações da mesma forma que fizemos com os recursos.
Uma ação é um objeto que contém um nome e relações entre recursos.
Por exemplo, uma ação de vender madeira remove uma certa quantidade de madeira do estoque e adiciona uma certa quantidade de moedas.
Por isso, vamos criar dois tipos: troca e ação.

1. Adicione as seguintes interfaces ao arquivo `types.ts`:

```tsx
/* types.ts */

export interface Trade {
  resource: Resource;
  quantity: number;
}

export interface Action {
  name: string;
  icon: string;
  trades: Trade[];
}

export interface Actions {
  chopWood: Action;
  sellWood: Action;
}
```

2. Edite o componente `ActionButton` para que ele receba um objeto do tipo `Action`.

```tsx
/* ActionButton.tsx */

interface ActionButtonProps {
  action: Action;
}
```

3. Atualize os props recebidos pela função que define o componente. Também atualize o conteúdo dos elementos `<p>` e `<span>` para exibir o nome e o ícone da ação, respectivamente.

```tsx
/* ActionButton.tsx */

export default function ActionButton({ action }: ActionButtonProps) {
  return (
    <div className={styles.box}>
      <p className={styles.name}>{action.name}</p>
      <button className={styles.button}>
        <span className={styles.icon}>{action.icon}</span>
      </button>
    </div>
  );
}
```

4. Dentro da **função** que define o componente `ActionsContainer`, crie um objeto do tipo `Action` para representar as ações de cortar madeira e vender madeira. Também crie um objeto do tipo `Actions` que inclua essas ações.

```tsx
/* ActionsContainer.tsx */

export default function ActionsContainer() {
  const chopWood: Action = {
    name: "Cortar Madeira",
    icon: "🪓",
    trades: [],
  };

  const sellWood: Action = {
    name: "Vender Madeira",
    icon: "💰",
    trades: [],
  };

  const actions: Actions = {
    chopWood,
    sellWood,
  };

  // ...
}
```

5. Atualize as chamadas do componente `ActionButton` para que elas passem as ações.

```tsx
/* ActionsContainer.tsx */

return (
  <section className={styles.container}>
    <div className={styles.group}>
      <ActionButton action={actions.chopWood} />
      <ActionButton action={actions.sellWood} />
    </div>
  </section>
);
```

O resultado final deve ser como o seguinte:

<Figure
  caption={`Recursos "Madeira" e "Moeda", e botões de ação para cortar madeira e vender madeira.`}
  images={[
    {
      src: actions,
      alt: `Página do projeto mostrando os recursos "Madeira" e "Moeda", e botões de ação para cortar e vender madeira, cada um com seu nome e ícone.`,
    },
  ]}
/>

## Elevação de estado

Você pode ter percebido que não preenchemos o vetor de trocas das ações.
Isso ocorre porque, para fazê-lo, é necessário receber a lista de todos os recursos disponíveis.
Entretanto, os recursos são definidos no componente `ResourcesContainer`, e as ações são definidas no componente `ActionsContainer`, que são irmãos na hierarquia de componentes.
Em situações como essa, devemos **elevar o estado** para o componente pai em comum.

1. Remova a definição do objeto `resources` do componente `ResourcesContainer` e o mova para o componente `App`.

```tsx
/* App.tsx */

export default function App() {
  const wood: Resource = {
    name: "Madeira",
    icon: "🌳",
    amount: 0,
  };

  const coin: Resource = {
    name: "Moeda",
    icon: "🪙",
    amount: 0,
  };

  const resources: Resources = {
    wood,
    coin,
  };

  // ...
}
```

2. Passe o objeto `resources` como propriedade para os componentes `ResourcesContainer` e `ActionsContainer`.

```tsx
/* App.tsx */

return (
  <article className={styles.container}>
    <Header />
    <ResourcesContainer resources={resources} />
    <ActionsContainer resources={resources} />
  </article>
);
```

3. Receba o objeto `resources` como propriedade no componente `ResourcesContainer`.

```tsx
/* ResourcesContainer.tsx */

interface ResourcesContainerProps {
  resources: Resources;
}

export default function ResourcesContainer({
  resources,
}: ResourcesContainerProps) {
  // ...
}
```

4. Receba o objeto `resources` como propriedade no componente `ActionsContainer`.

```tsx
/* ActionsContainer.tsx */

interface ActionsContainerProps {
  resources: Resources;
}

export default function ActionsContainer({ resources }: ActionsContainerProps) {
  // ...
}
```

5. Atualize as propriedades `trades` das ações para que elas recebam a lista de recursos disponíveis.

```tsx
/* ActionsContainer.tsx */

const chopWood: Action = {
  name: "Cortar Madeira",
  icon: "🪓",
  trades: [{ resource: resources.wood, quantity: +1 }],
};

const sellWood: Action = {
  name: "Vender Madeira",
  icon: "💰",
  trades: [
    { resource: resources.wood, quantity: -1 },
    { resource: resources.coin, quantity: +1 },
  ],
};
```

## Hook useState

Precisamos de alguma forma de atualizar o valor dos recursos quando uma ação é executada.
Como fazer isso se os recursos são definidos no componente `App` e o botão está no componente `ActionButton`?

O React fornece um **hook** chamado `useState` que permite adicionar estado a um componente funcional.
Ele permite que, ao atualizar o estado, o componente seja renderizado novamente para refletir as mudanças.

<Aside type="note">
  Um **hook** é uma função especial que implementa diferentes funcionalidades do
  React em componentes funcionais.
</Aside>

1. Importe o hook `useState` do React no arquivo `App.tsx`.

```tsx
/* App.tsx */

import { useState } from "react";
```

2. Apague as definições dos objetos `resource` e `resources`. Então use o hook `useState` para criar um estado que armazena os dados definidos anteriormente.

```tsx
/* App.tsx */

export default function App() {
  const [resources, setResources] = useState<Resources>({
    wood: {
      name: "Madeira",
      icon: "🌳",
      amount: 0,
    },
    coin: {
      name: "Moeda",
      icon: "🪙",
      amount: 0,
    },
  });

  // ...
}
```

<Aside type="note">
  O hook `useState` retorna um array com dois elementos. O primeiro é o estado
  atual, que é inicializado com o valor passado como argumento. O segundo é uma
  função que atualiza o estado, a qual pode ser chamada para inserir um novo
  valor.
</Aside>

3. Passe a função `setResources` como propriedade para o componente `ActionsContainer`.

```tsx
/* App.tsx */

<ActionsContainer resources={resources} setResources={setResources} />
```

4. Receba a função `setResources` como propriedade no componente `ActionsContainer`.

```tsx
/* ActionsContainer.tsx */

interface ActionsContainerProps {
  resources: Resources;
  setResources: React.Dispatch<React.SetStateAction<Resources>>;
}
```

5. Atualize a função que define o componente `ActionsContainer` para que ela receba a função `setResources`.

```tsx
export default function ActionsContainer({
  resources,
  setResources,
}: ActionsContainerProps) {
  // ...
}
```

6. Passe a função `setResources` como propriedade para os componentes `ActionButton`.

```tsx
/* ActionsContainer.tsx */

<ActionButton action={actions.chopWood} setResources={setResources} />
<ActionButton action={actions.sellWood} setResources={setResources} />
```

7. Receba a função `setResources` como propriedade no componente `ActionButton`.

```tsx
/* ActionButton.tsx */

interface ActionButtonProps {
  action: Action;
  setResources: React.Dispatch<React.SetStateAction<Resources>>;
}
```

8. Atualize os props recebidos pela função que define o componente `ActionButton`.

```tsx
/* ActionButton.tsx */

export default function ActionButton({
  action,
  setResources,
}: ActionButtonProps) {
  // ...
}
```

## Atualizando os recursos

Vamos atualizar a função `performTrade` no componente `ActionButton` para que ela calcule as trocas e atualize o estado dos recursos.

1.  Inicialize uma variável para determinar se é possível executar todas as trocas necessárias para a ação.

```tsx
/* ActionButton.tsx */

const performTrade = () => {
  let tradeIsPossible = true;

  // ...
};
```

2.  Itere sobre as trocas necessárias para a ação, recebendo pelos parâmetros o recurso e a quantidade a ser alterada. Podemos utilizar a sintaxe de desestruturação para acessá-los.

```tsx
/* ActionButton.tsx */

action.trades.forEach(({ resource, quantity }) => {
  // ...
});
```

<Aside type="note">
  O método `forEach` é utilizado para iterar sobre os elementos de um vetor. Ele
  recebe uma função de *callback* nos parâmetros, a qual executa sobre cada
  elemento do vetor. Como somos nós quem está chamando o método, podemos definir
  o corpo da função de *callback*. Ela pode receber pelos parâmetros: o elemento
  atual, o índice do elemento e o vetor completo.
</Aside>

3.  Dentro do `forEach`, calcule o novo valor do recurso e verifique se a troca é possível.

```tsx
/* ActionButton.tsx */

const newAmount = resource.amount + quantity;
tradeIsPossible = tradeIsPossible && newAmount >= 0;
```

4.  Dentro do `forEach`, caso a troca seja possível, atualize o valor do recurso.

```tsx
/* ActionButton.tsx */

if (tradeIsPossible) resource.amount = newAmount;
```

5.  Após o `forEach`, verifique se todas as trocas foram possíveis e, caso positivo, atualize o estado dos recursos.

```tsx
/* ActionButton.tsx */

if (tradeIsPossible) setResources(resources => ({ ...resources }));
```

<Aside type="note">
  A função `setResources` recebe uma função de *callback* que retorna o novo
  estado. Ela passa como parâmetro o valor do estado antes de fazermos a
  atualização. Então, utilizamos o operador de
  [**espalhamento**](https://developer.mozilla.org/pt-BR/docs/Web/JavaScript/Reference/Operators/Spread_syntax)
  (`...`) para copiar todos os recursos atualizados para o estado existente.
</Aside>

O código final da função `performTrade` deve ser igual ao seguinte:

```tsx
/* ActionButton.tsx */

const performTrade = () => {
  let tradeIsPossible = true;

  action.trades.forEach(({ resource, quantity }) => {
    const newAmount = resource.amount + quantity;
    tradeIsPossible = tradeIsPossible && newAmount >= 0;

    if (tradeIsPossible) resource.amount = newAmount;
  });

  if (tradeIsPossible) setResources(resources => ({ ...resources }));
};
```

Ao clicar no botão de cortar madeira, a quantidade de madeira deve aumentar em uma unidade.
Já ao clicar no botão de vender madeira, a quantidade de madeira deve diminuir em uma unidade e a quantidade de moedas deve aumentar em uma unidade.

## Criando novos recursos e ações

Agora que temos um sistema de recursos e ações flexível, podemos adicionar novos recursos e ações facilmente.
Porém, antes disso, precisamos ajustar um detalhe.
O jogador não tem como saber quais são as quantidades envolvidas nas trocas!

### Lista de trocas

Para resolver isso, vamos editar o componente `ActionButton` para exibir as quantidades de troca.
Após o elemento `<button>`, crie um elemento de lista não ordenada [`<ul>`](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/ul).

```tsx
/* ActionButton.tsx */

<ul className={styles.trades}>
  {action.trades.map(({ resource, quantity }) => (
    <li key={resource.name}>
      {quantity > 0 ? "+" : ""}
      {quantity} {resource.icon} {resource.name}
    </li>
  ))}
</ul>
```

O resultado final deve ser como o seguinte:

<Figure
  caption="Botões de ação para cortar madeira e vender madeira."
  images={[
    {
      src: trades,
      alt: `Página do projeto mostrando os botões de ação para cortar e vender madeira, cada um com seu nome, ícone e trocas associadas. A troca de cortar madeira é de +1 🌳 Madeira, e a troca de vender madeira é de -1 🌳 Madeira e +1 🪙 Moeda.`,
    },
  ]}
/>

### Recursos

Agora, vamos adicionar dois novos recursos: casa e trabalhador.

1. Edite o tipo `Resources` no arquivo `types.ts` para incluir os novos recursos.

```tsx
/* types.ts */

export interface Resources {
  wood: Resource;
  coin: Resource;
  house: Resource;
  worker: Resource;
}
```

2. Adicione os objetos `house` e `worker` ao estado inicial de recursos no componente `App`.

```tsx
/* App.tsx */

const [resources, setResources] = useState<Resources>({
  wood: {
    name: "Madeira",
    icon: "🌳",
    amount: 0,
  },
  coin: {
    name: "Moeda",
    icon: "🪙",
    amount: 0,
  },
  house: {
    name: "Casa",
    icon: "🏠",
    amount: 0,
  },
  worker: {
    name: "Trabalhador",
    icon: "👷",
    amount: 0,
  },
});
```

3. Edite o componente `ResourcesContainer` para exibir os novos recursos dinamicamente.

```tsx
/* ResourcesContainer.tsx */

return (
  <section className={styles.container}>
    <div className={styles.group}>
      {Object.values(resources).map(resource => (
        <ResourceDisplay key={resource.name} resource={resource} />
      ))}
    </div>
  </section>
);
```

O resultado final deve ser como o seguinte:

<Figure
  caption="Recursos Madeira, Moeda, Casa e Trabalhador."
  images={[
    {
      src: newResources,
      alt: `Página do projeto mostrando os recursos "Madeira", "Moeda", "Casa" e "Trabalhador", cada um com seu nome, ícone e quantidade. Abaixo, há botões de ação para cortar e vender madeira.`,
    },
  ]}
/>

### Ações

Vamos adicionar duas novas ações: construir casa e contratar trabalhador.
A casa custa madeira e moedas.
Já o trabalhador precisa do salário pago em moedas e de uma casa para morar.

1. Edite o tipo `Actions` no arquivo `types.ts` para incluir as novas ações.

```tsx
/* types.ts */

export interface Actions {
  chopWood: Action;
  sellWood: Action;
  buildHouse: Action;
  hireWorker: Action;
}
```

2. Adicione os objetos `buildHouse` e `hireWorker` ao estado inicial de ações no componente `ActionsContainer`.

```tsx
/* ActionsContainer.tsx */

const buildHouse: Action = {
  name: "Construir Casa",
  icon: "🛠️",
  trades: [
    { resource: resources.wood, quantity: -5 },
    { resource: resources.coin, quantity: -5 },
    { resource: resources.house, quantity: +1 },
  ],
};

const hireWorker: Action = {
  name: "Contratar Trabalhador",
  icon: "🧾",
  trades: [
    { resource: resources.coin, quantity: -10 },
    { resource: resources.house, quantity: -1 },
    { resource: resources.worker, quantity: +1 },
  ],
};

const actions: Actions = {
  chopWood,
  sellWood,
  buildHouse,
  hireWorker,
};
```

3. Atualize as chamadas do componente `ActionButton` para que elas passem as novas ações.

```tsx
/* ActionsContainer.tsx */

return (
  <section className={styles.container}>
    <div className={styles.group}>
      {Object.values(actions).map(action => (
        <ActionButton
          key={action.name}
          action={action}
          setResources={setResources}
        />
      ))}
    </div>
  </section>
);
```

O resultado final deve ser como o seguinte:

<Figure
  caption="Botões de ação para cortar madeira, vender madeira, construir casa e contratar trabalhador."
  images={[
    {
      src: newActions,
      alt: `Página do projeto mostrando os recursos "Madeira", "Moeda", "Casa" e "Trabalhador", cada um com seu nome, ícone e quantidade. Abaixo, há os seguintes botões de ação: "Cortar Madeira", "Vender Madeira", "Construir Casa" e "Contratar Trabalhador", cada um com seu nome, ícone e trocas associadas.`,
    },
  ]}
/>
