...
<Aside type="note">
  A sintaxe de **desestruturação** é uma forma de extrair valores de objetos e
  vetores e atribuí-los a variáveis. Para utilizá-la, basta colocar o nome do
  objeto entre chaves `{}` &mdash; para vetores, utilizamos colchetes `[]`.
</Aside>
...
<Aside type="note">
  A função `performTrade` é uma função de flecha &mdash; ou **arrow function**.
